...
from SimPEG.utils import surface2ind_topo, mkvc, speye
...
# ==========================================================================================================
domain_numbers = [6000.0, 8000.0, 10000.0, 12000.0]
data_dir = r'data/Calculation Domain Filtering/'
for domain_num in domain_numbers:
    proj_name = f'{domain_num}domain-5'
    bz_fname = ''.join([data_dir, proj_name, '_bz.dat'])
    dbz_fname = ''.join([data_dir, proj_name, '_dbz.dat'])
    t_fname = ''.join([data_dir, proj_name, '_t.dat'])
    fig_fname = ''.join([data_dir, proj_name, '.png'])
    ...
    dom_width = domain_num      # Total domain width (meters)
    ...
    num_y=1                 # Number of receivers along y-axis (single point)
    xrx, yrx = np.meshgrid(np.linspace(0, 0, num_x),
                         np.linspace(-0, 0, num_y))
    ...
    np.savetxt('model_bf.dat',model)         # Save background model to current directory
    np.savetxt('model_af.dat',model)         # No target body, background model is final model
    ...
    if plot_mesh0:
        ...
        # ======================
        # Custom parameter section (unit: centimeters)
        # ======================
        fig_width_cm = 11.6  # Figure width (centimeters)
        fig_height_cm = 7.73  # Figure height (centimeters)
        x_min, x_max = 0, 100  # x-axis range (customizable)
        y_min, y_max = 0, 100  # y-axis range (customizable)

        # [Core conversion] Centimeters to inches (1 inch = 2.54 cm)
        ...
        # Set axis limits (customizable)
        x_min, x_max = -400, 400  # Custom x-axis range
        y_min, y_max = -400, 400  # Custom y-axis range
        ax1.set_xlim(x_min, x_max)
        ax1.set_ylim(y_min, y_max)

        # [x-axis label font] Chinese/English can be configured separately
        xlabel_font = {'family': 'Times New Roman', 'size': 28}
        ylabel_font = {'family': 'Times New Roman', 'size': 28}
        # [Colorbar title font] Usually English
        ...
        vmin = np.min(log_model) - 1  # Decrease minimum by one order of magnitude (log10 space)
        vmax = np.max(log_model) + 1  # Increase maximum by one order of magnitude (log10 space)
        ...
        # Custom tick label format as $10^{%.1f}$
        ...
        plt.savefig('80lft_mesh.png', dpi=600, pad_inches=0)
        plt.show()
    # ==========================================================================================================
    ...
    np.savetxt(t_fname, t_off)           # Save time series to custom filename
    ...
    gamma=7e-5           # Shift parameter (shifted inverse Arnoldi) - smaller shift parameter
    ...
    s3time=time.time()
    np.savetxt(bz_fname, bz_SD.T)  # Save magnetic field data to custom filename
    bz_SD = np.loadtxt(bz_fname).reshape(t_off.shape[0],1)  # Reshape to column vector
    ...
    etime = time.time()
    np.savetxt(dbz_fname, dbdt_SD_off)  # Save time derivative data to custom filename
